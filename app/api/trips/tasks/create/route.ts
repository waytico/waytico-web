...
export async function POST(req: Request) {
  ...
  const { projectId, title, type, deadline } = body;
  ...
  try {
    const res = await fetch(`${TRIP_MANAGER_URL}/api/trips/tasks/create`, {
      method: "POST",
      headers: {
    ...
    if (!res.ok) {
      ...
      return Response.json(
        { error: err.error || "Failed to create task" },
        { status: res.status }
      );
    }
    ...
    console.error("[TaskCreate] Error:", e?.message);
